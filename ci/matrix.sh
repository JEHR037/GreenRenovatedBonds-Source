...
export BUILDER_IMAGE_NAME="greenrenovatedbonds-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export USE_SHELL="/bin/greenrenovatedbonds"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_GRB_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true
