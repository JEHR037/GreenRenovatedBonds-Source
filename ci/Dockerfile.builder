...
# greenrenovatedbonds_hash
RUN git clone https://github.com/greenrenovatedbondspay/greenrenovatedbonds_hash
RUN cd greenrenovatedbonds_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} greenrenovatedbonds
RUN useradd -u ${USER_ID} -g greenrenovatedbonds -s /bin/bash -m -d /greenrenovatedbonds greenrenovatedbonds
...
RUN mkdir /greenrenovatedbonds-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /greenrenovatedbonds-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /greenrenovatedbonds-src

USER greenrenovatedbonds
